...
\be
	\item rewriting $C$ as $C =\{x \in  \mathbf{R}^{n} | (x^T A x) + (b^T x) \le \alpha, \alpha \in   \mathbf{R} \}$.
	...
	%\item Using the result of BV Ex 2.2 we will show that the intersection of C with a line is convex when $A \succeq 0$.
	...
	%$C \cap l = \{ x | \alpha t^2 + \beta t + \gamma \text{ where } \alpha = v^T A v, \beta = 2 x^T A v + b^T v \text{ and } \gamma = x^T A x + b^T x + c \le 0\}$.
	...
	%In conclusion, $C \cap l$ is convex when $\alpha =  v^T A v \ge 0$ thus C is convex when $A \succeq 0$.
