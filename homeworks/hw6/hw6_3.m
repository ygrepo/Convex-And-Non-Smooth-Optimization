%% HW 6 − Nesterov's example (p.67) − M=100 & M=10000
clear all;
clc;
...
tmp = (M+m)/(M-m);
xstar(2,1) = (2*tmp*xstar(3)+1)/(4*tmp^2-1);
xstar(1,1) = (1 + xstar(2))/(2*tmp);
[pstar,gstar,xstar] = fun(xstar);
...
semilogy(err_ratio_grad1, "b-x", 'LineWidth',1,'MarkerSize',3);
hold on
...
semilogy(err_ratio_grad2, "b-x", 'LineWidth',1,'MarkerSize',3);
hold on

% Plot upper bound
upper_bd_grad2 = kappa *( (1  - 1/kappa) / (1+1/kappa) ).^(2*t);
